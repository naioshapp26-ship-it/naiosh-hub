...
(function () {
  ...
  var TITLES = {
    ...
    catalog: ['المتجر والكتالوج', 'منتجات · خدمات · أنظمة'],
  ...
  function renderCommand(cmd) {
    ...
    return '<div class="pos-kpis">' +
      ...
      '<div class="pos-grid-2">' +
        ...
        '<section class="pos-card"><h3>تجديدات قريبة</h3><div class="pos-list">' +
          ((cmd.renewals || []).length ? cmd.renewals.map(function (r) {
            return '<div class="pos-row"><div><strong>' + esc(r.systemName) + '</strong><small>' + esc(r.clientName) + ' · ' + esc(fmt(r.renewsAt)) + '</small></div></div>';
      ...
      '<section class="pos-card"><h3>ملخص CRM</h3><p class="pos-muted">عملاء: ' + esc(s.totalClients || 0) +
        ...
        ' · مشاكل: ' + esc(s.openIssues || 0) + '</p></section>';
  }

  function renderClients(list) {
    return '<section class="pos-card"><h3>عملاء بوشا</h3>' +
      '<div class="pos-table-wrap"><table class="pos-table"><thead><tr>' +
      '<th>العميل</th><th>الحالة</th><th>Lifecycle</th><th>Health</th><th>أنظمة</th><th>طلبات</th><th>شكاوى</th><th></th>' +
      '</tr></thead><tbody>' +
      (list || []).map(function (c) {
        return '<tr>' +
          ...
          '<td><a class="pos-btn" href="dashboard.html#posha-clients">360</a></td>' +
          '</tr>';
      }).join('') +
    ...
    return '<section class="pos-card"><h3>صندوق العملاء</h3><div class="pos-list" id="pos-inbox-list">' +
      ((list || []).length ? list.map(function (i) {
        return '<div class="pos-row" data-inbox="' + esc(i.id) + '"><div><strong>' + esc(i.title) + '</strong>' +
          ...
          '<div>' + badge(i.status) + '<div class="pos-actions"><button type="button" class="pos-btn primary" data-assign="' + esc(i.id) + '">تعيين لي</button></div></div></div>';
    ...
    return '<section class="pos-card"><h3>الشكاوى</h3><div class="pos-list">' +
      ((list || []).length ? list.map(function (c) {
        return '<div class="pos-row"><div><strong>' + esc(c.number) + ' — ' + esc(c.subject) + '</strong>' +
          '<small>' + esc(c.clientName || c.clientEmail) + ' · ' + esc(c.category) + '</small></div>' + badge(c.status) + '</div>';
  ...
  function renderTasks(list) {
    return '<section class="pos-card"><h3>المهام</h3><div class="pos-list">' +
      ((list || []).length ? list.map(function (t) {
        return '<div class="pos-row"><div><strong>' + esc(t.title) + '</strong><small>استحقاق: ' + esc(fmt(t.due_date)) + ' · ' + esc(t.related_client || '') + '</small></div>' + badge(t.status) + '</div>';
