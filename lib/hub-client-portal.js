...
function blankClient(email, name = '') {
  ...
  return {
    ...
    orders: [],
    subscriptions: [],
    wallet: { paid: 0, free: 300, total: 300, ledger: [] },
    invoices: [],
    tickets: [],
    notifications: [],
    activity: [],
...
function blankStore() {
  return { version: 1, clients: {}, activity: [], updatedAt: new Date().toISOString() };
}
...
function marketplaceCatalog() {
  return [
    ...
    },
    {
      ...
      code: 'LMS',
      name: 'نظام التعلم',
      description: 'دورات وتدريب الفرق',
      priceLabel: 'من 79$',
      logo: 'assets/logo-hub.jpeg',
      ctaUrl: 'packages.html',
      moreUrl: 'packages.html',
    },
    {
      id: 'store',
      code: 'STORE',
      name: 'متجر هوب',
      description: 'منتجات وخدمات جاهزة',
...
function touchLogin(email, name) {
...
async function ensureDemoClientAccount() {
  ...
  if (!store.clients[email] || !(store.clients[email].systems || []).length) {
    ...
    if (!(store.clients[email].systems || []).length) {
      ...
      store.clients[email].wallet = store.clients[email].wallet || { paid: 0, free: 300, total: 300, ledger: [] };
    }
    writeStore(store);
  }
}
...
async function handleClientApi(req, res, pathname) {
  ...
  if (pathname === '/api/client/tickets' && req.method === 'POST') {
    ...
    const ticket = {
      ...
      department: String(body.department || 'دعم فني'),
      priority: String(body.priority || 'medium'),
      status: 'open',
      createdAt: new Date().toISOString(),
      messages: [{ id: uid('msg'), at: new Date().toISOString(), from: 'client', body: message }],
    };
    me().tickets.unshift(ticket);
    me().notifications.unshift({
      id: uid('ntf'),
      title: 'تم فتح تذكرة دعم',
      body: subject,
      at: new Date().toISOString(),
      read: true,
      kind: 'support',
    });
    logActivity(store, { action: 'TICKET_CREATED', user_id: session.email, actor_id: session.email, metadata: { ticketId: ticket.id } });
    writeStore(store);
  ...
  if (pathname.startsWith('/api/client/tickets/') && req.method === 'POST') {
    ...
    const body = await readBody(req);
    const ticket = (me().tickets || []).find((t) => t.id === id);
    if (!ticket) {
    ...
    const message = String(body.message || body.body || '').trim();
    if (!message) {
      sendJson(res, 400, { ok: false, error: 'الرسالة مطلوبة' });
      return true;
    }
    ticket.messages.push({ id: uid('msg'), at: new Date().toISOString(), from: 'client', body: message });
  ...
  if (pathname === '/api/client/security/password' && req.method === 'POST') {
    ...
    logActivity(store, { action: 'PASSWORD_CHANGE', user_id: session.email, actor_id: session.email });
    writeStore(store);
...
async function handleAdminClientsApi(req, res, pathname) {
  ...
  if (parts[0] === 'api' && parts[1] === 'admin' && parts[2] === 'clients' && parts[3] && parts[4] === 'status' && req.method === 'POST') {
    ...
    if (client.status === 'suspended') {
      logActivity(store, { action: 'ACCOUNT_SUSPENDED', user_id: email, actor_id: session.email });
    }
    writeStore(store);
    sendJson(res, 200, { ok: true, client: publicClient(client, { includeInternal: true }) });
    return true;
