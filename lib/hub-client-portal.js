...
async function handleClientApi(req, res, pathname) {
  ...
  if (pathname === '/api/client/invoices/pay' && req.method === 'POST') {
    ...
    const wallet = me().wallet || { paid: 0, free: 0, total: 0, ledger: [] };
    const amount = Number(inv.amount || 0);
    if ((wallet.total || 0) < amount) {
      posha().emitEvent(store, {
        type: 'PAYMENT_FAILED',
        clientEmail: session.email,
        actorId: session.email,
        actorRole: 'client',
        title: `فشل دفع الفاتورة ${inv.number}`,
      ...
      writeStore(store);
      sendJson(res, 400, { ok: false, error: 'رصيد المحفظة غير كافٍ لدفع الفاتورة' });
      return true;
    }
    wallet.paid = Math.max(0, (wallet.paid || 0) - amount);
    wallet.total = (wallet.paid || 0) + (wallet.free || 0);
    wallet.ledger.unshift({
      id: uid('led'),
      at: new Date().toISOString(),
      type: 'debit',
      amount,
      note: `دفع فاتورة ${inv.number}`,
    });
    me().wallet = wallet;
    inv.status = 'paid';
    posha().emitEvent(store, {
      ...
      actorId: session.email,
      actorRole: 'client',
      title: `تم دفع الفاتورة ${inv.number}`,
      message: `${amount} ${inv.currency || ''}`.trim(),
      metadata: { invoiceId: inv.id },
    });
    writeStore(store);
    sendJson(res, 200, { ok: true, invoice: inv, wallet });
    return true;
  }
