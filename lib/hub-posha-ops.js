/**
 * عملاء هوب — shared ops layer (single source of truth on client-portal store).
 * CLIENT <-> store <-> ADMIN
 */
'use strict';
 ...
 */
const EVENT_POLICY = {
  CLIENT_REGISTERED: { activity: true, client: true, admin: true, issue: false },
  CLIENT_APPROVED: { activity: true, client: true, admin: true, issue: false },
  ...
  ORDER_COMPLETED: { activity: true, client: true, admin: true, issue: false },
  ORDER_AWAITING_PAYMENT: { activity: true, client: true, admin: true, issue: false },
  INVOICE_CREATED: { activity: true, client: true, admin: false, issue: false },
  ...
  SUBSCRIPTION_EXPIRED: { activity: true, client: true, admin: true, issue: true },
  SUBSCRIPTION_RENEWED: { activity: true, client: true, admin: true, issue: false },
  ...
  REQUEST_CREATED: { activity: true, client: true, admin: true, issue: false },
  REQUEST_STATUS_CHANGED: { activity: true, client: true, admin: false, issue: false },
  COMPLAINT_CREATED: { activity: true, client: true, admin: true, issue: false },
  COMPLAINT_STATUS_CHANGED: { activity: true, client: true, admin: false, issue: false },
  COMPLAINT_ESCALATED: { activity: true, client: true, admin: true, issue: true },
  ...
  TASK_CREATED: { activity: true, client: false, admin: true, issue: false },
  TASK_ASSIGNED: { activity: true, client: false, admin: true, issue: false },
  TASK_COMPLETED: { activity: true, client: false, admin: true, issue: false },
  TASK_OVERDUE: { activity: true, client: false, admin: true, issue: true },
  SLA_BREACHED: { activity: true, client: false, admin: true, issue: true },
  ONBOARDING_UPDATED: { activity: true, client: true, admin: false, issue: false },
...
function deepLinkFor(type, meta = {}) {
  ...
  if (type.startsWith('REQUEST')) return { entityType: 'request', entityId: meta.requestId || meta.id, adminHash: 'posha-os', adminTab: 'requests' };
  if (type.startsWith('COMPLAINT')) return { entityType: 'complaint', entityId: meta.complaintId || meta.id, adminHash: 'posha-os', adminTab: 'complaints' };
  if (type.startsWith('INVOICE')) return { entityType: 'invoice', entityId: meta.invoiceId || meta.id, adminHash: 'posha-clients', adminTab: 'invoices' };
  if (type.startsWith('SYSTEM') || type.startsWith('SUBSCRIPTION')) {
    return { entityType: 'system', entityId: meta.systemCode || meta.id, adminHash: 'posha-clients', adminTab: 'systems' };
  }
...
async function handlePoshaAdminApi(req, res, pathname) {
  ...
  if (parts[3] === 'clients' && parts[4] && parts[5] === 'status' && req.method === 'POST') {
    if (!hubSession.hasPermission(session, 'users.suspend') && !hubSession.hasPermission(session, 'customers.manage') && !hubSession.hasPermission(session, 'orders.update')) {
      // allow ops admins with wallet or systems too
      if (!hubSession.hasPermission(session, 'systems.assign') && !hubSession.hasPermission(session, 'wallet.adjust')) {
        sendJson(res, 403, { ok: false, error: 'ليست لديك صلاحية تغيير حالة العميل' });
        return true;
      }
    }
    const email = decodeURIComponent(parts[4]).toLowerCase();
    ...
    if (!['active', 'suspended', 'pending', 'closed'].includes(next)) {
      sendJson(res, 400, { ok: false, error: 'حالة غير صالحة' });
      return true;
    }
    ...
    emitEvent(store, {
      type: next === 'suspended' ? 'CLIENT_SUSPENDED' : 'CLIENT_STATUS_CHANGED',
      clientEmail: email,
      actorId: session.email,
      actorRole: 'admin',
      title: `تغيير حالة العميل ${client.name || email}`,
      message: `${prev} → ${next}`,
      metadata: { clientId: client.clientId, status: next },
    });
    portal().writeStore(store);
    sendJson(res, 200, { ok: true, client: clientRow(client) });
    return true;
  }
