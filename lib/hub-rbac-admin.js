...
const PERMISSION_LEVELS = [
  {
    ...
    color: '#94a3b8',
    priority: 0,
    summary_ar: 'لا يرى هذا النظام ولا يمكنه العمل عليه.',
    risk: 'low',
  },
  {
    code: 'VIEW',
    ...
    color: '#6B7280',
    priority: 1,
    summary_ar: 'يمكنه رؤية المعلومات دون تعديلها.',
    risk: 'low',
  },
  {
    code: 'READ_WRITE',
    name_ar: 'قراءة وكتابة',
    name_en: 'Read/Write',
    color: '#3B82F6',
    priority: 2,
    summary_ar: 'يمكنه إضافة وتعديل السجلات اليومية.',
    risk: 'medium',
  },
  {
    code: 'EXECUTIVE',
    ...
    color: '#8B5CF6',
    priority: 3,
    summary_ar: 'يشغّل العمليات الإدارية داخل النظام.',
    risk: 'medium',
  },
  {
    code: 'APPROVE',
    ...
    color: '#F59E0B',
    priority: 4,
    summary_ar: 'يعتمد المعاملات والطلبات — صلاحية حسّاسة.',
    risk: 'high',
  },
  {
    code: 'FULL_ACCESS',
    name_ar: 'صلاحية كاملة',
    name_en: 'Full Access',
    color: '#EF4444',
    priority: 5,
    summary_ar: 'تحكم كامل بما فيه الإعدادات والحذف — صلاحية عالية الخطورة.',
    risk: 'critical',
  },
];

/** أدوار محمية من الحذف العرضي */
const PROTECTED_ROLE_CODES = new Set(['SUPER_ADMIN']);

const ROLE_AUDIENCE = {
  SUPER_ADMIN: 'مالك المنصة والمسؤول الأعلى',
  IT_MANAGER: 'فريق التقنية والدعم',
  HQ_EXECUTIVE_MANAGER: 'الإدارة التنفيذية في المقر',
  HQ_FINANCIAL_MANAGER: 'المالية والمحاسبة',
  HQ_MARKETING_MANAGER: 'التسويق والنمو',
  HQ_PROCUREMENT_MANAGER: 'المشتريات والتوريد',
  HQ_PR_MANAGER: 'العلاقات العامة',
  LEGAL_MANAGER: 'الشؤون القانونية',
  CONTENT_MANAGER: 'المحتوى والتحرير',
  INITIATIVES_MANAGER: 'المبادرات والبرامج',
  FREELANCER_MANAGER: 'إدارة المستقلين',
  BRANCH_MANAGER: 'مدراء الفروع',
  INCUBATOR_MANAGER: 'مدراء الحاضنات',
  PLATFORM_MANAGER: 'مدراء المنصات',
  OFFICE_EXECUTIVE: 'المكاتب الإلكترونية',
  EDITOR: 'فريق التحرير',
  LOGISTICS_EMPLOYEE: 'اللوجستيات والعمليات',
  PERMANENT_TRAINER: 'المدربون',
  FREELANCER_TRAINER: 'المدربون المستقلون',
  VOLUNTEER_TRAINER: 'المتطوعون',
  INITIATIVES_VOLUNTEER: 'متطوعو المبادرات',
};

function roleAudience(code) {
  if (ROLE_AUDIENCE[code]) return ROLE_AUDIENCE[code];
  if (/BRANCH/.test(code)) return 'الفروع';
  if (/INCUBATOR/.test(code)) return 'الحاضنات';
  if (/PLATFORM/.test(code)) return 'المنصات';
  if (/OFFICE/.test(code)) return 'المكاتب';
  if (/MANAGER|ADMIN|EXECUTIVE/.test(code)) return 'الإدارة والتشغيل';
  if (/TRAINER|VOLUNTEER|EDITOR/.test(code)) return 'التشغيل الميداني والتدريب';
  return 'الموظفون والمستخدمون';
}

function rolePlainDescription(role) {
  const title = role.title_ar || role.name_ar || role.code;
  const audience = roleAudience(role.code);
  if (role.code === 'SUPER_ADMIN') {
    return 'يملك تحكمًا كاملاً بكل أنظمة هوب — دور محمي ولا يُحذف.';
  }
  if (role.description && !String(role.description).startsWith('دور ')) {
    return role.description;
  }
  return `مخصّص لـ ${audience}. يحدّد ما يمكن لـ«${title}» رؤيته وفعله داخل أنظمة نايوش.`;
}

function enrichRole(store, role) {
  const perms = Array.isArray(role.permissions) ? role.permissions : [];
  const activePerms = perms.filter((p) => p.permission_level && p.permission_level !== 'NONE');
  const usersCount = (store.users || []).filter((u) => u.role_code === role.code && u.is_active !== false).length;
  const highest = activePerms.reduce((max, p) => {
    const lvl = PERMISSION_LEVELS.find((l) => l.code === p.permission_level);
    return Math.max(max, lvl ? lvl.priority : 0);
  }, 0);
  const highestLevel = PERMISSION_LEVELS.find((l) => l.priority === highest) || PERMISSION_LEVELS[0];
  const isSystem = PROTECTED_ROLE_CODES.has(role.code) || ROLE_SEED.some((s) => s.code === role.code);
  return {
    ...role,
    users_count: usersCount,
    systems_count: activePerms.length,
    permissions_count: activePerms.length,
    plain_description: rolePlainDescription(role),
    audience: roleAudience(role.code),
    is_system: isSystem,
    is_protected: PROTECTED_ROLE_CODES.has(role.code),
    is_sensitive: highest >= 4 || PROTECTED_ROLE_CODES.has(role.code),
    highest_level: highestLevel.code,
    highest_level_ar: highestLevel.name_ar,
    can_delete: !PROTECTED_ROLE_CODES.has(role.code),
    can_edit: true,
...
function blankStore() {
  const roles = ROLE_SEED.map((r, i) => ({
    ...
    title_ar: r.title_ar,
    title_en: r.code,
    description: rolePlainDescription({ code: r.code, title_ar: r.title_ar }),
...
async function handleAdminApi(req, res, pathname) {
  ...
  if (pathname === '/api/admin/metadata' && req.method === 'GET') {
    sendJson(res, 200, {
      ...
      help: {
        role: 'الدور مجموعة صلاحيات تُمنح لمستخدم أو أكثر لتحدد ماذا يرى ويفعل.',
        system_role: 'دور مدمج في النظام ومحمي من الحذف العرضي.',
        permission_level: 'يحدد مستوى العمل داخل كل نظام: عرض، تعديل، اعتماد، أو تحكم كامل.',
        sensitive: 'صلاحيات الاعتماد والتحكم الكامل تؤثر على بيانات حساسة أو إعدادات النظام.',
      },
      templates: [
        { code: 'VIEWER', title_ar: 'مشاهد', description: 'عرض فقط عبر الأنظمة', base_level: 'VIEW' },
        { code: 'EMPLOYEE', title_ar: 'موظف', description: 'قراءة وكتابة للعمليات اليومية', base_level: 'READ_WRITE' },
        { code: 'MANAGER', title_ar: 'مدير', description: 'تشغيل وإدارة عمليات النظام', base_level: 'EXECUTIVE' },
        { code: 'APPROVER', title_ar: 'معتمد', description: 'اعتماد المعاملات الحساسة', base_level: 'APPROVE' },
        { code: 'ADMIN', title_ar: 'مسؤول نظام', description: 'صلاحية كاملة — استخدم بحذر', base_level: 'FULL_ACCESS' },
        { code: 'CUSTOM', title_ar: 'دور مخصص', description: 'ابدأ من الصفر وحدد الصلاحيات بنفسك', base_level: 'NONE' },
      ],
    });
    return true;
  }
  ...
  if (pathname === '/api/admin/roles' && req.method === 'GET') {
    const roles = store.roles.map((r) => enrichRole(store, r));
  ...
  if (pathname === '/api/admin/roles' && req.method === 'POST') {
    ...
    const role = {
      ...
      permissions: Array.isArray(body.permissions) && body.permissions.length
        ...
        : body.template_level
          ? SYSTEMS.map((s) => ({
              system_code: s.code,
              permission_level: body.template_level === 'NONE' ? 'NONE' : String(body.template_level),
            }))
    ...
    writeStore(store);
    sendJson(res, 201, { success: true, role: enrichRole(store, role) });
    return true;
  }
  ...
  if (parts[0] === 'api' && parts[1] === 'admin' && parts[2] === 'roles' && parts[3] && parts[4] === 'permissions' && req.method === 'PUT') {
    ...
    const changed = body.change_summary || { permissions: role.permissions.length };
    logAudit(store, 'UPDATE', changed, 'role_permissions', role.code);
    writeStore(store);
    sendJson(res, 200, { success: true, role: enrichRole(store, role), permissions: role.permissions });
    return true;
  }
  ...
  if (parts[0] === 'api' && parts[1] === 'admin' && parts[2] === 'roles' && parts[3] && req.method === 'PUT' && !parts[4]) {
    ...
    writeStore(store);
    sendJson(res, 200, { success: true, role: enrichRole(store, role) });
    return true;
  }

  if (parts[0] === 'api' && parts[1] === 'admin' && parts[2] === 'roles' && parts[3] && req.method === 'DELETE' && !parts[4]) {
    const target = findRole(store, parts[3]);
    if (!target) {
      sendJson(res, 404, { success: false, message: 'الدور غير موجود' });
      return true;
    }
    if (PROTECTED_ROLE_CODES.has(target.code)) {
      sendJson(res, 403, {
        success: false,
        message: 'هذا دور نظام محمي ولا يمكن حذفه. يمكنك تعديل بعض إعداداته أو إنشاء دور مخصص بدلًا منه.',
      });
      return true;
    }
    const assigned = store.users.filter((u) => u.role_code === target.code).length;
    if (assigned > 0 && url.searchParams.get('force') !== '1') {
      sendJson(res, 409, {
        success: false,
        message: `هذا الدور معيّن لـ ${assigned} مستخدمًا. أكّد الحذف لإزالة الربط.`,
        users_count: assigned,
        requires_force: true,
      });
      return true;
    }
    store.users.forEach((u) => {
      if (u.role_code === target.code) u.role_code = null;
    });
    store.roles = store.roles.filter((r) => String(r.code) !== String(target.code));
    logAudit(store, 'DELETE', { title: target.title_ar, users_unassigned: assigned }, 'roles', target.code);
    writeStore(store);
    sendJson(res, 200, { success: true });
    return true;
  }
